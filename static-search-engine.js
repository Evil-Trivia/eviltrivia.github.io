...
class StaticSearchEngine {
    ...
    variableMatch(pattern, text) {
        ...
        // Original simple variable matching for patterns without $
    ...
    variableMatchWithWildcards(pattern, text, patternIndex, textIndex, variables) {
        ...
        // Base case: reached end of text but not pattern
        ...
        if (patternChar === '$') {
            ...
            for (let len = 1; len <= text.length - textIndex; len++) {
                ...
                if (this.variableMatchWithWildcards(pattern, text, patternIndex + 1, textIndex + len, newVariables)) {
                    return true;
                }
            }
            
            return false;
        } else if (/[A-Z]/.test(patternChar.toUpperCase())) {
            // Single character variable
            const upperChar = patternChar.toUpperCase();
            const textChar = text[textIndex].toUpperCase();
            
            if (variables.has(upperChar)) {
                // Check if it matches the stored value
                if (variables.get(upperChar) !== textChar) {
                    return false;
                }
            } else {
                // Store the variable value
                variables.set(upperChar, textChar);
            }
            
            return this.variableMatchWithWildcards(pattern, text, patternIndex + 1, textIndex + 1, variables);
        } else {
            // Literal character
            if (patternChar.toUpperCase() !== text[textIndex].toUpperCase()) {
                return false;
            }
